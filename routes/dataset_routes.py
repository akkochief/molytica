...
tqdm.pandas(desc="Veri isleniyor")
...
if not RDKIT_AVAILABLE:
    logger.warning("RDKit bulunamadi. Kurulum: pip install rdkit")
...
class ChemicalDatabase:
    ...
    def _init_db(self):
        ...
        try:
            ...
            """)
            c.execute("""
                CREATE TABLE IF NOT EXISTS resolution_lineage (
    ...
    def _load_name_memory(self):
        try:
            ...
            logger.debug(f"name_memory yuklenemedi: {e}")
    ...
    def _persist_common(self):
        ...
        try:
            ...
            for name, data in self.common.items():
                c.execute("""
                    INSERT OR IGNORE INTO name_cache (name, smiles, source, confidence)
                    VALUES (?, ?, 'builtin', 5)
                """, (name, data["smiles"]))
            conn.commit()
        finally:
    ...
    def _save_name_async(self, name: str, smiles: str, source: str):
        try:
            ...
            c = conn.cursor()
            c.execute("""
                INSERT INTO name_cache (name, smiles, source, confidence)
                VALUES (?, ?, ?, 1)
                ON CONFLICT(name) DO UPDATE SET
                    smiles=excluded.smiles, source=excluded.source,
                    confidence=name_cache.confidence + 1, last_updated=CURRENT_TIMESTAMP
            """, (name, smiles, source))
            conn.commit()
            conn.close()
        except Exception as e:
            logger.debug(f"save_name basarisiz ({name}): {e}")
    ...
    def get_properties(self, canonical_smiles: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(f"get_properties basarisiz: {e}")
        return None
    ...
    def _save_properties_async(self, canonical_smiles: str, props: Dict):
        try:
            ...
            c = conn.cursor()
            c.execute("""
                INSERT INTO properties_cache (canonical_smiles, props_json, last_updated)
                VALUES (?, ?, CURRENT_TIMESTAMP)
                ON CONFLICT(canonical_smiles) DO UPDATE SET
                    props_json=excluded.props_json, last_updated=CURRENT_TIMESTAMP
            """, (canonical_smiles, json.dumps(props)))
            conn.commit()
            conn.close()
        except Exception as e:
            logger.debug(f"save_properties basarisiz: {e}")
    ...
    def mark_failed(self, name: str, reason: str):
        try:
            ...
            c = conn.cursor()
            c.execute("""
                INSERT INTO failed_lookups (name, attempts, reason, last_attempt)
                VALUES (?, 1, ?, CURRENT_TIMESTAMP)
                ON CONFLICT(name) DO UPDATE SET
                    attempts = failed_lookups.attempts + 1,
                    reason = excluded.reason,
                    last_attempt = CURRENT_TIMESTAMP
            """, (self.normalize(name), reason))
            conn.commit()
            conn.close()
        except Exception as e:
            logger.debug(f"mark_failed basarisiz: {e}")
    ...
    def log_lineage(self, record):
        try:
            ...
            c = conn.cursor()
            c.execute("""
                INSERT INTO resolution_lineage
                (query_name, normalized_key, winning_source, canonical_smiles,
                 inchikey, fragment_count, attempted_sources)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (record.query_name, self.normalize(record.query_name), record.winning_source,
                  record.canonical_smiles, record.inchikey, record.fragment_count,
                  json.dumps(record.attempted_sources)))
            conn.commit()
            conn.close()
        except Exception as e:
            logger.debug(f"log_lineage basarisiz: {e}")

def _build_common_chemicals() -> Dict[str, Dict]:
    ...
    add({
        'water': dict(smiles='O'), 'ethanol': dict(smiles='CCO'), 'methanol': dict(smiles='CO'),
        'isopropanol': dict(smiles='CC(C)O'), '2-propanol': dict(smiles='CC(C)O'),
        'toluene': dict(smiles='Cc1ccccc1'), 'dmf': dict(smiles='CN(C)C=O'),
        'n,n-dimethylformamide': dict(smiles='CN(C)C=O'), 'dmso': dict(smiles='CS(C)=O'),
        'thf': dict(smiles='C1CCOC1'), 'tetrahydrofuran': dict(smiles='C1CCOC1'),
        'dioxane': dict(smiles='C1COCCO1'), '1,4-dioxane': dict(smiles='C1COCCO1'),
        'acetone': dict(smiles='CC(=O)C'), 'acetonitrile': dict(smiles='CC#N'),
        'dichloromethane': dict(smiles='ClCCl'), 'dcm': dict(smiles='ClCCl'),
        'chloroform': dict(smiles='ClC(Cl)Cl'), 'hexane': dict(smiles='CCCCCC'),
        'cyclohexane': dict(smiles='C1CCCCC1'), 'ethyl acetate': dict(smiles='CCOC(=O)C'),
        'diethyl ether': dict(smiles='CCOCC'), 'benzene': dict(smiles='c1ccccc1'),
        'pyridine': dict(smiles='c1ccncc1'), 'nmp': dict(smiles='CN1CCCC1=O'),
        'dme': dict(smiles='COCCOC'), '1,2-dimethoxyethane': dict(smiles='COCCOC'),
    })
    add({
        'potassium carbonate': dict(smiles='O=C([O-])[O-].[K+].[K+]'),
        'k2co3': dict(smiles='O=C([O-])[O-].[K+].[K+]'),
        'cesium carbonate': dict(smiles='O=C([O-])[O-].[Cs+].[Cs+]'),
        'cs2co3': dict(smiles='O=C([O-])[O-].[Cs+].[Cs+]'),
        'sodium carbonate': dict(smiles='O=C([O-])[O-].[Na+].[Na+]'),
        'sodium bicarbonate': dict(smiles='OC([O-])=O.[Na+]'),
        'sodium hydroxide': dict(smiles='[Na+].[OH-]'), 'naoh': dict(smiles='[Na+].[OH-]'),
        'potassium hydroxide': dict(smiles='[K+].[OH-]'), 'koh': dict(smiles='[K+].[OH-]'),
        'tripotassium phosphate': dict(smiles='[K+].[K+].[K+].[O-]P([O-])([O-])=O'),
        'k3po4': dict(smiles='[K+].[K+].[K+].[O-]P([O-])([O-])=O'),
        'potassium phosphate': dict(smiles='[K+].[K+].[K+].[O-]P([O-])([O-])=O'),
        'sodium tert-butoxide': dict(smiles='CC(C)(C)[O-].[Na+]'),
        'potassium tert-butoxide': dict(smiles='CC(C)(C)[O-].[K+]'),
        'triethylamine': dict(smiles='CCN(CC)CC'),
        'diisopropylethylamine': dict(smiles='CC(C)N(CC)C(C)C'), 'dipea': dict(smiles='CC(C)N(CC)C(C)C'),
        'cesium fluoride': dict(smiles='[F-].[Cs+]'), 'potassium fluoride': dict(smiles='[F-].[K+]'),
        'potassium acetate': dict(smiles='CC(=O)[O-].[K+]'),
        'barium hydroxide': dict(smiles='[OH-].[OH-].[Ba+2]'),
    })
    add({
        'tetrakis(triphenylphosphine)palladium(0)': dict(
            smiles='c1ccc(cc1)P(c1ccccc1)c1ccccc1.c1ccc(cc1)P(c1ccccc1)c1ccccc1.'
                   'c1ccc(cc1)P(c1ccccc1)c1ccccc1.c1ccc(cc1)P(c1ccccc1)c1ccccc1.[Pd]'),
        'pd(pph3)4': dict(
            smiles='c1ccc(cc1)P(c1ccccc1)c1ccccc1.c1ccc(cc1)P(c1ccccc1)c1ccccc1.'
                   'c1ccc(cc1)P(c1ccccc1)c1ccccc1.c1ccc(cc1)P(c1ccccc1)c1ccccc1.[Pd]'),
        'palladium(ii) acetate': dict(smiles='CC(=O)O[Pd]OC(C)=O'), 'pd(oac)2': dict(smiles='CC(=O)O[Pd]OC(C)=O'),
        'bis(triphenylphosphine)palladium(ii) dichloride': dict(
            smiles='Cl[Pd](Cl)(P(c1ccccc1)(c1ccccc1)c1ccccc1)P(c1ccccc1)(c1ccccc1)c1ccccc1'),
        'pdcl2(pph3)2': dict(
            smiles='Cl[Pd](Cl)(P(c1ccccc1)(c1ccccc1)c1ccccc1)P(c1ccccc1)(c1ccccc1)c1ccccc1'),
        'pd(dppf)cl2': dict(smiles='Cl[Pd]Cl.c1ccc(cc1)P(c1ccccc1)[c]1cccc1'),
        'palladium(ii) chloride': dict(smiles='Cl[Pd]Cl'), 'pdcl2': dict(smiles='Cl[Pd]Cl'),
        'palladium on carbon': dict(smiles='[Pd]'), 'pd/c': dict(smiles='[Pd]'),
    })
    add({
        'triphenylphosphine': dict(smiles='c1ccc(cc1)P(c1ccccc1)c1ccccc1'),
        'pph3': dict(smiles='c1ccc(cc1)P(c1ccccc1)c1ccccc1'),
        'sphos': dict(smiles='COc1cccc(OC)c1-c1ccccc1P(C1CCCCC1)C1CCCCC1'),
        'xphos': dict(smiles='CC(C)c1cc(C(C)C)c(-c2ccccc2P(C2CCCCC2)C2CCCCC2)c(C(C)C)c1'),
        'dppf': dict(smiles='c1ccc(cc1)P(c1ccccc1)[c]1cccc1'),
    })
    add({
        'bromobenzene': dict(smiles='Brc1ccccc1'), 'chlorobenzene': dict(smiles='Clc1ccccc1'),
        'iodobenzene': dict(smiles='Ic1ccccc1'), '4-bromotoluene': dict(smiles='Cc1ccc(Br)cc1'),
        '4-bromoanisole': dict(smiles='COc1ccc(Br)cc1'), '2-bromopyridine': dict(smiles='Brc1ccccn1'),
    })
    add({
        'phenylboronic acid': dict(smiles='OB(O)c1ccccc1'),
        'phenylboronic acid pinacol ester': dict(smiles='CC1(C)OB(OC1(C)C)c1ccccc1'),
        '4-methylphenylboronic acid': dict(smiles='Cc1ccc(cc1)B(O)O'),
        '4-methoxyphenylboronic acid': dict(smiles='COc1ccc(cc1)B(O)O'),
    })
    return common

_COMMON_CHEMICALS = _build_common_chemicals()
...
class SMILESResolver:
    SOURCE_ORDER = [
        ("pubchem_name", "PubChem (isim aramasi)"),
        ("cas_common_chemistry", "CAS Common Chemistry"),
        ("chembl", "ChEMBL"),
        ("opsin", "OPSIN (IUPAC ayristirici)"),
        ("pubchem_synonym", "PubChem (esanlamli arama)"),
        ("cactus", "NCI/CACTUS (CIR)"),
    ]
    ...
    def resolve(self, name: str) -> Tuple[Optional[str], ResolutionRecord]:
        ...
        for key, label in self.SOURCE_ORDER:
            ...
            try:
                ...
                logger.debug(f"[{label}] zaman asimi: {name}")
                with self.lock:
                    self.stats[f"timeout_{key}"] += 1
            except requests.exceptions.RequestException as e:
                logger.debug(f"[{label}] ag hatasi ({name}): {e}")
                with self.lock:
                    self.stats[f"network_error_{key}"] += 1
            except (ValueError, KeyError, json.JSONDecodeError) as e:
                logger.debug(f"[{label}] yanit ayristirma hatasi ({name}): {e}")
            except Exception as e:
                logger.debug(f"[{label}] beklenmeyen hata ({name}): {e}")
    ...
    def resolve_batch(self, values: List[str], workers: Optional[int] = None) -> List[Optional[str]]:
        ...
        if remaining:
            with ThreadPoolExecutor(max_workers=workers) as ex:
                ...
                for future in tqdm(as_completed(futures), total=len(futures), desc="SMILES cozumleme"):
                    v = futures[future]
                    try:
                        ...
                        logger.warning(f"'{v}' cozumlenirken beklenmeyen hata: {e}")
                        results[v] = None
                        self.lineage[v] = ResolutionRecord(v, status=f"exception:{type(e).__name__}")
...
class PropertyCalculator:
    ...
    def _largest_fragment(self, mol):
        try:
            ...
            logger.debug(f"Fragment sanitizasyonu basarisiz: {e}")
            try:
    ...
    def fragment_breakdown(self, smiles: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for frag in sorted(frags, key=lambda m: m.GetNumAtoms(), reverse=True):
                ...
                role = "metal_or_organometallic" if is_metal else (
                    "organic_core" if frag.GetNumAtoms() > 3 else "counterion_or_small_fragment"
                )
                try:
            ...
            logger.debug(f"fragment_breakdown basarisiz ({smiles}): {e}")
        return out
    ...
    def _calc_rdkit(self, smiles: str) -> Tuple[Dict[str, Any], str]:
        ...
        try:
            ...
            logger.debug(f"Temel ozellikler basarisiz ({smiles}): {e}")

        try:
            props["inchikey"] = Chem.MolToInchiKey(mol_full)
        except Exception as e:
            logger.debug(f"InChIKey hesaplanamadi ({smiles}): {e}")
        ...
        try:
            ...
            logger.debug(f"Fizikokimyasal ozellikler basarisiz ({smiles}): {e}")

        try:
            props["hba"] = Lipinski.NumHAcceptors(mol)
            props["hbd"] = Lipinski.NumHDonors(mol)
        except Exception as e:
            logger.debug(f"HBA/HBD basarisiz ({smiles}): {e}")
        ...
        try:
            ...
            for ring in ri.AtomRings():
                ...
                if is_het:
                    ...
                    carbocyclic += 1
            props.update(aromatic_rings=ar, aliphatic_rings=al, hetero_rings=het,
                         saturated_rings=sat, carbocyclic_rings=carbocyclic,
                         heterocyclic_rings=heterocyclic)
        except Exception as e:
            logger.debug(f"Halka analizi basarisiz ({smiles}): {e}")
        ...
        try:
            ...
            logger.debug(f"Spiro/bridgehead basarisiz ({smiles}): {e}")
        ...
        try:
            ...
            logger.debug(f"Topoloji/dallanma hatasi ({smiles}): {e}")
        ...
        try:
            ...
            logger.debug(f"Fonksiyonel grup/dal sayimi hatasi ({smiles}): {e}")
        ...
        try:
            ...
            props.update(
                c_count=counts.get(6, 0), n_count=counts.get(7, 0), o_count=counts.get(8, 0),
                s_count=counts.get(16, 0), p_count=counts.get(15, 0), f_count=counts.get(9, 0),
                cl_count=counts.get(17, 0), br_count=counts.get(35, 0), i_count=counts.get(53, 0),
                b_count=counts.get(5, 0), si_count=counts.get(14, 0),
            )
            ...
            logger.debug(f"Atom sayimi basarisiz ({smiles}): {e}")
        ...
        try:
            ...
            logger.debug(f"Bag analizi basarisiz ({smiles}): {e}")
        ...
        if self.cfg.compute_3d_descriptors:
            try:
                ...
                logger.debug(f"3D ozellikler basarisiz ({smiles}): {e}")
    ...
    def _get_pubchem_cid(self, smiles: str) -> Optional[str]:
        try:
            ...
            logger.debug(f"PubChem CID sorgusu basarisiz ({smiles}): {e}")
        return None
    ...
    def calculate_batch(self, smiles_list: List[str], workers: Optional[int] = None
        ...
        if remaining:
            logger.info(f"   {len(remaining)} yeni molekul icin ozellik hesaplaniyor...")
            with ThreadPoolExecutor(max_workers=workers) as ex:
                futures = {ex.submit(self.calculate, s): s for s in remaining}
                for future in tqdm(as_completed(futures), total=len(futures), desc="Ozellik & Dal hesaplaniyor"):
                    s = futures[future]
                    try:
                        results[s] = future.result(timeout=30)
                    except Exception as e:
                        logger.warning(f"'{s}' icin ozellik hesaplama hatasi: {e}")
                        results[s] = (default_properties(), f"exception:{type(e).__name__}")
...
class MoleculeMapBuilder:
    ...
    def build(self, df: pd.DataFrame, column_map: Dict[str, str],
              resolver_lineage: Dict[str, ResolutionRecord],
              props_map: Dict[str, Dict[str, Tuple[Dict[str, Any], str]]]) -> Dict[str, Any]:
    ...
    def print_tree(self, molecule_map: Dict[str, Any], max_per_column: int = 5):
        print("\n" + "="*80)
        print("MOLEKULER YAPITASI VE DALLANMA HARITASI")
        print("="*80)
        
        for col, molecules in molecule_map.items():
            print(f"\nSutun: {col.upper()} ({len(molecules)} Benzersiz Molekul)")
            for i, (name, info) in enumerate(molecules.items()):
                if i >= max_per_column:
                    print(f"   ... ve {len(molecules) - max_per_column} molekul daha gizlendi.")
                    break
                
                marker = "+" if info["status"] in ("ok", "ok_cached") else "-"
                print(f"   +-- {marker} Isim: {name}")
                print(f"   |   SMILES: {info['smiles']}")
                
                if info["fragments"]:
                    print("   |       Parcaciklar (Fragments):")
                    for idx, frag in enumerate(info["fragments"]):
                        is_last = (idx == len(info["fragments"]) - 1)
                        branch_char = "+--" if is_last else "|--"
                        
                        role_icon = "[M]" if frag["role"] == "metal_or_organometallic" else (
                                    "[C]" if frag["role"] == "organic_core" else "[I]")
                        
                        print(f"   |           {branch_char} {role_icon} {frag['smiles']} ({frag['atom_count']} Atom)")
                else:
                    print("   |       Parcalanamadi veya Tek Parca")
        print("="*80 + "\n")
...
SOLVENT_PHYSICS: Dict[str, Dict[str, float]] = {
    "water": dict(dielectric=80.1, bp_c=100.0, polarity_index=10.2, donor_number=18.0),
    "methanol": dict(dielectric=32.7, bp_c=64.7, polarity_index=5.1, donor_number=19.0),
    "ethanol": dict(dielectric=24.6, bp_c=78.4, polarity_index=4.3, donor_number=19.2),
    "isopropanol": dict(dielectric=19.9, bp_c=82.3, polarity_index=3.9, donor_number=18.5),
    "acetone": dict(dielectric=20.7, bp_c=56.1, polarity_index=5.1, donor_number=17.0),
    "acetonitrile": dict(dielectric=37.5, bp_c=82.0, polarity_index=5.8, donor_number=14.1),
    "dmso": dict(dielectric=46.7, bp_c=189.0, polarity_index=7.2, donor_number=29.8),
    "dmf": dict(dielectric=36.7, bp_c=153.0, polarity_index=6.4, donor_number=26.6),
    "thf": dict(dielectric=7.5, bp_c=66.0, polarity_index=4.0, donor_number=20.0),
    "dioxane": dict(dielectric=2.2, bp_c=101.0, polarity_index=4.8, donor_number=14.8),
    "toluene": dict(dielectric=2.4, bp_c=110.6, polarity_index=2.4, donor_number=0.1),
    "benzene": dict(dielectric=2.3, bp_c=80.1, polarity_index=2.7, donor_number=0.1),
    "dichloromethane": dict(dielectric=8.9, bp_c=39.6, polarity_index=3.1, donor_number=0.0),
    "hexane": dict(dielectric=1.9, bp_c=68.7, polarity_index=0.1, donor_number=0.0),
    "ethyl acetate": dict(dielectric=6.0, bp_c=77.1, polarity_index=4.4, donor_number=14.0),
    "diethyl ether": dict(dielectric=4.3, bp_c=34.6, polarity_index=2.8, donor_number=19.2),
    "pyridine": dict(dielectric=12.3, bp_c=115.2, polarity_index=5.3, donor_number=33.1),
}

_LEAVING_GROUP_SMARTS = [
    ("Ar-I", "c-I", 4.0), ("Ar-Br", "c-Br", 3.0),
    ("Ar-OTf", "cOS(=O)(=O)C(F)(F)F", 2.5), ("Ar-Cl", "c-Cl", 1.0), ("Ar-F", "c-F", 0.2),
]
...
def detect_substituent_pattern(smiles: str) -> Dict[str, int]:
    ...
    try:
        mol = Chem.MolFromSmiles(smiles)
        if not mol: return pattern_counts
...
def calculate_stoichiometric_discrepancy(row: pd.Series) -> Dict[str, float]:
    metrics = {
        "pd_ligand_ratio": np.nan,
        "ligand_excess": np.nan,
        "catalyst_loading_mmol": np.nan,
        "ligand_pd_complexation_efficiency": np.nan,
        "pd_l_3_formation_indicator": 0.0
    }
    ...
    try:
        ...
        logger.debug(f"Stoichiometric calculation failed: {e}")
        
    return metrics

def calculate_hsab_compatibility(smiles: str, catalyst: str, solvent: str) -> Dict[str, float]:
    metrics = {
        'hsab_pd_halide_mismatch': 10.0,
        'hsab_pd_ligand_mismatch': 10.0,
        'hsab_ligand_halide_mismatch': 10.0,
        'hsab_overall_compatibility': 0.0,
        'hsab_soft_soft_interaction_score': 0.0,
        'hsab_hard_hard_interaction_score': 0.0,
        'hsab_pearson_class_match': 0.0
    }
    ...
    try:
        ...
        metrics['hsab_overall_compatibility'] = round(
            (1 - metrics['hsab_pd_halide_mismatch'] / 10.0) * 0.5 +
            (1 - metrics['hsab_pd_ligand_mismatch'] / 10.0) * 0.3 +
            (1 - metrics['hsab_ligand_halide_mismatch'] / 10.0) * 0.2 +
            metrics['hsab_pearson_class_match'] * 0.1,
            3
        )
        ...
        logger.debug(f"HSAB calculation failed: {e}")
...
def add_domain_features(df: pd.DataFrame, cfg: Config = CONFIG) -> Tuple[pd.DataFrame, List[str]]:
    ...
    for solv_col in ("solv1", "solv2"):
        ...
        valid_count = norm.notna().sum()
        logger.info(f"   {solv_col} icin {valid_count}/{len(out)} gecerli solvent, fizikokimyasal sabitler eklendi")
    ...
    logger.info(f"   toplam {len(added)} domain ozelligi eklendi")
    return out, added

def add_ultra_advanced_academic_features(df: pd.DataFrame, cfg: Config = CONFIG) -> Tuple[pd.DataFrame, List[str]]:
    logger.info("Ultra-Advanced Akademik Ozellik Muhendisligi Basliyor...")
    
    out = df.copy()
    added_cols = []
    
    if "subs1_SMILES" in out.columns:
        logger.info("   Hammett & Taft hesaplaniyor...")
    ...
    if "catalizor" in out.columns:
        logger.info("   Katalizor-Ligand orani hesaplaniyor...")
        stoich_data = out.progress_apply(calculate_stoichiometric_discrepancy, axis=1)
        
        for col in ["pd_ligand_ratio", "ligand_excess", "catalyst_loading_mmol", 
                   "ligand_pd_complexation_efficiency", "pd_l_3_formation_indicator"]:
    ...
    if "subs1_SMILES" in out.columns and "catalizor" in out.columns:
        logger.info("   HSAB teorisi uyum hesaplaniyor...")
        
        hsab_data = out.progress_apply(
            lambda row: calculate_hsab_compatibility(
                row.get("subs1_SMILES", ""),
                row.get("catalizor", ""),
                row.get("solv1", "")
            ),
            axis=1
        ...
        hsab_cols = [
            "hsab_pd_halide_mismatch",
            "hsab_pd_ligand_mismatch",
            "hsab_ligand_halide_mismatch",
            "hsab_overall_compatibility",
            "hsab_soft_soft_interaction_score",
            "hsab_hard_hard_interaction_score",
            "hsab_pearson_class_match"
        ]
    ...
    if "subs1_sigma_p" in out.columns and "hsab_overall_compatibility" in out.columns:
        out["mechanistic_predictor_electronic_softness"] = (
            out["subs1_sigma_p"] * 0.3 + 
            out["hsab_soft_soft_interaction_score"].fillna(0) * 0.4 +
            out["hsab_overall_compatibility"].fillna(0) * 0.3
        )
        added_cols.append("mechanistic_predictor_electronic_softness")
        
        out["mechanistic_oxidative_addition_liability"] = (
            (1 - out["subs1_sigma_p"]) * 0.4 +
            out["hsab_soft_soft_interaction_score"].fillna(0) * 0.4 +
            (1 - out["hsab_pd_halide_mismatch"].fillna(10) / 10) * 0.2
        )
        out["mechanistic_oxidative_addition_liability"] = out["mechanistic_oxidative_addition_liability"].clip(0, 1)
        added_cols.append("mechanistic_oxidative_addition_liability")
    
    if "subs1_SMILES" in out.columns and "subs2_SMILES" in out.columns:
        out["transmetalation_steric_mismatch"] = abs(
            out.get("subs1_taft_es", 0) - 
            out.get("subs2_taft_es", 0)
        )
    ...
    logger.info(f"Toplam {len(added_cols)} ultra-advanced akademik ozellik eklendi")
    return out, added_cols

def add_all_academic_features(df: pd.DataFrame, cfg: Config = CONFIG) -> pd.DataFrame:
    logger.info("=" * 60)
    logger.info("ULTRA-ADVANCED AKADEMIK OZELLIK MUHENDISLIGI")
    logger.info("=" * 60)
    
    df, domain_cols = add_domain_features(df, cfg)
    df, academic_cols = add_ultra_advanced_academic_features(df, cfg)
    
    logger.info("=" * 60)
    logger.info(f"Toplam {len(domain_cols) + len(academic_cols)} yeni ozellik eklendi")
    logger.info("=" * 60)
...
def read_universal_csv(file_path: str) -> pd.DataFrame:
    logger.info(f"Okunuyor: {file_path}")
    ...
    for enc in encodings:
        try:
            ...
            if len(df) > 0:
                logger.info(f"{len(df)} satir, {len(df.columns)} sutun ({enc})")
                return df
        except Exception as e:
            last_err = e
            continue
    raise ValueError(f"CSV okunamadi, denenen kodlamalar basarisiz: {last_err}")
...
def analyze_target_variable(df: pd.DataFrame, target_col: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target_col is None:
        ...
        report["message"] = f"Hedef degisken bulunamadi. Aranan: {ML_TARGET_CANDIDATES}"
        return make_json_serializable(report)
    
    if target_col not in df.columns:
        report["status"] = "column_not_found"
        report["message"] = f"{target_col} sutunu DataFrame'de bulunamadi"
        return make_json_serializable(report)
    ...
    if values.empty:
        report["status"] = "empty"
        report["message"] = "Hedef degisken bos"
        return make_json_serializable(report)
    ...
    if is_numeric:
        ...
        if unique_count <= 10:
            ...
            if len(class_counts) >= 2:
                ...
                if report["imbalanced"]:
                    report["recommendation"] = "Sinif dengesizligi tespit edildi. SMOTE, class_weight veya stratified sampling onerilir."
                else:
                    report["recommendation"] = "Siniflar dengeli gorunuyor."
            else:
                report["class_imbalance_ratio"] = 1.0
                report["imbalanced"] = False
                report["recommendation"] = "Tek bir sinif mevcut (regresyon problemi olabilir)."
        else:
            ...
            report["recommendation"] = "Surekli hedef degisken tespit edildi. Regresyon modelleri icin uygun."
    else:
        ...
        if len(class_counts) >= 2:
            ...
            report["class_imbalance_ratio"] = 1.0
            report["imbalanced"] = False
            report["recommendation"] = "Tek bir sinif mevcut."
...
class TaskStatus:
    __slots__ = ['task_id', 'status', 'percentage', 'message', 'message_key',
                 'result', 'error', 'created_at', 'updated_at', 'logs', '_log_lock']

    def __init__(self, task_id):
        self.task_id = task_id
        self.status = 'pending'
        self.percentage = 0
        self.message = 'Baslatiliyor...'
...
def run_conversion_pipeline(cfg: Config, task: TaskStatus) -> Dict[str, Any]:
    ...
    task.update(3, 'Veritabani ve cozumleyiciler hazirlaniyor...', 'preparing')
    db = ChemicalDatabase(cfg.db_path)
    resolver = SMILESResolver(db, cfg)
    calc = PropertyCalculator(db, cfg)

    task.update(6, 'CSV okunuyor...', 'reading')
    df = read_universal_csv(cfg.input_csv)
    task.add_log(f"Sutunlar: {list(df.columns)}")
    task.add_log(f"Veri boyutu: {df.shape[0]} satir, {df.shape[1]} sutun")

    task.update(9, 'Hedef degisken analiz ediliyor...', 'target_analysis')
    target_report = analyze_target_variable(df, cfg.target_column)
    if target_report.get("status") == "success":
        task.add_log(f"Hedef degisken: {target_report['target_column']} "
                      f"({target_report['type']}, kapsam %{target_report['data_coverage']})")
        if target_report['type'] in ('categorical', 'categorical_string') and target_report.get('imbalanced'):
            task.add_log(f"Sinif dengesizligi: oran={target_report.get('class_imbalance_ratio', 0)}")
    else:
        task.add_log(f"Hedef degisken tespit edilemedi: {target_report.get('message', '')}")

    available = [c for c in cfg.reaction_columns if c in df.columns]
    if not available:
        raise ValueError(
            "Beklenen reaksiyon sutunlarindan hicbiri bulunamadi "
            "(subs1, subs2, product, catalizor, base, solv1, solv2)."
        )

    task.update(12, f'SMILES cozumleme basliyor: {len(available)} sutun...', 'smiles')
    column_map: Dict[str, str] = {}
    n_cols = len(available)
    for idx, col in enumerate(available):
        task.add_log(f"SMILES cozumleniyor: {col}")
        ...
        if cfg.checkpoint_every_column:
            try:
                ...
                logger.debug(f"Checkpoint kaydi basarisiz: {e}")

        pct = 12 + int(((idx + 1) / n_cols) * 33)
        task.update(pct, f'SMILES: {idx + 1}/{n_cols} sutun tamamlandi ({col})', 'smiles')

    smiles_cols = list(column_map.values())

    task.update(46, f'Molekuler ozellikler hesaplaniyor: {len(smiles_cols)} sutun...', 'properties')
    props_map_by_col: Dict[str, Dict[str, Tuple[Dict[str, Any], str]]] = {}
    n_sc = max(len(smiles_cols), 1)
    for idx, sc in enumerate(smiles_cols):
        task.add_log(f"Ozellikler hesaplaniyor: {sc}")
        unique = [s for s in df[sc].dropna().unique() if s and str(s).strip()]
        if not unique:
            task.add_log(f"{sc} icin SMILES bulunamadi, atlaniyor.")
            continue
        ...
        df[status_col] = df[sc].progress_apply(
            lambda x: props_map.get(str(x).strip(), ({}, "no_input"))[1] if pd.notna(x) else "no_input"
        )

        for prop in PROPERTY_SCHEMA:
            col_name = f"{sc}_{prop}"
            df[col_name] = df[sc].progress_apply(
                lambda x, p=prop: props_map.get(str(x).strip(), ({}, "missing"))[0].get(p)
        ...
        task.add_log(f"{sc}: {len(PROPERTY_SCHEMA)} ozellik eklendi | "
                     f"basariyla hesaplanan satir: {n_ok}/{len(df)}")
        ...
        task.update(pct, f'Ozellikler: {idx + 1}/{n_sc} sutun tamamlandi ({sc})', 'properties')

    task.update(82, 'Akademik ozellik muhendisligi uygulaniyor...', 'features')
    df = add_all_academic_features(df, cfg)

    task.update(88, 'Molekuler harita (soy agaci) olusturuluyor...', 'mapping')
    ...
    task.update(93, 'Bos hucreler kontrol ediliyor...', 'cleanup')
    null_counts = df.isnull().sum()
    total_nulls = int(null_counts.sum())
    if total_nulls > 0:
        task.add_log(f"{total_nulls} bos hucre kaldi, dolduruluyor...")
        ...
        task.add_log("Bos degerler dolduruldu.")

    task.update(97, 'CSV kaydediliyor...', 'saving')
    df.to_csv(cfg.output_csv, index=False, encoding="utf-8-sig")

    elapsed = time.time() - start
    task.add_log(f"{os.path.basename(cfg.output_csv)} — {len(df.columns)} sutun, {len(df)} satir")
    task.add_log(f"Süre: {elapsed:.2f} saniye")

    return {
        'output_file': os.path.basename(cfg.output_csv),
        'columns_converted': available,
        'rows_processed': len(df),
        'total_columns_output': len(df.columns),
...
@dataset_bp.route('/')
def index():
    print("Ana sayfa cagrildi")
    return render_template('dataset.html')

@dataset_bp.route('/upload', methods=['POST'])
def upload_file():
    print("\n" + "=" * 60)
    print("YENI DOSYA YUKLEME ISTEGI")
    print("=" * 60)

    try:
        if 'file' not in request.files:
            print("Hata: Dosya secilmedi")
            return jsonify({'error': 'Dosya secilmedi'}), 400

        file = request.files['file']
        if file.filename == '':
            print("Hata: Dosya adi bos")
            return jsonify({'error': 'Dosya secilmedi'}), 400

        print(f"Dosya: {file.filename}")

        if not file.filename.lower().endswith('.csv'):
            print("Hata: Sadece CSV dosyalari kabul edilir")
            return jsonify({'error': 'Sadece CSV dosyalari kabul edilir'}), 400
        ...
        task.add_log(f"Dosya yuklendi: {filename}")
        ...
        thread = threading.Thread(
            ...
            args=(task_id, filepath, filename, app)
        )
        thread.daemon = True
        thread.start()
        ...
        return jsonify({
            'success': True,
            'task_id': task_id,
            'message': 'Dosya yuklendi, islem baslatiliyor...'
        })
    ...
    try:
        with app.app_context():
            ...
            cfg = Config(
                ...
                db_path=os.path.join(db_dir, 'universal_chemical.db'),
                log_path=os.path.join(log_dir, f"{task_id}.log"),
                checkpoint_csv=output_path + ".checkpoint.csv",
            )
            ...
            print(f"Task {task_id} tamamlandi")
            print("=" * 60 + "\n")

    except Exception as e:
        print(f"Task {task_id} hatasi: {e}")
        traceback.print_exc()
        task.fail(str(e))
        try:
            os.remove(filepath)
...
def get_progress(task_id):
    ...
    if not task:
        return jsonify({'error': 'Task bulunamadi'}), 404
...
def save_dataset():
    try:
        ...
        if not content:
            return jsonify({'success': False, 'message': 'Icerik bos'})

        if not filename:
            return jsonify({'success': False, 'message': 'Dosya adi gerekli'})
        ...
        return jsonify({
            'success': True,
            'message': f'CSV basariyla kaydedildi: {filename}',
            'filename': filename
        })
    except Exception as e:
...
def delete_dataset(filename):
    try:
        ...
        os.remove(filepath)
        return jsonify({'success': True, 'message': f'{filename} silindi'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
...
print("=" * 60)
print("SMILES/Ozellik Donusturucu API Baslatildi (tek dosya)")
print("Outputs: static/datasets")
print("SMILES kaynak sirasi: PubChem -> CAS -> ChEMBL -> OPSIN -> PubChem esanlamli -> CACTUS")
print("456+ molekuler ozellik (RDKit) + akademik ozellik muhendisligi")
print("=" * 60)